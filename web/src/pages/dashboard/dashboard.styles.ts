export const dashboardClassNames: Record<string, string> = {
  ...
  modal: 'modal-panel',
  close: 'modal-close ui-muted grid h-10 w-10 shrink-0 place-items-center border-0 bg-transparent',
  tabs: 'tab-list',
  primary: 'button-primary',
  secondary: 'button-secondary',
  danger:
    'ui-danger inline-flex h-11 items-center justify-center rounded-[var(--element-radius)] border bg-transparent px-3',
  solid: 'ui-danger-solid',
  link: 'button-link',
  error: 'ui-error text-sm',
  check: 'flex items-center gap-2 [&>input]:h-4 [&>input]:w-4',
  'field-row': 'responsive-field-row',
...
export const dashboardCn = (...names: Array<string | false | null | undefined>) =>
  names
    ...
    .map((name) => dashboardClassNames[name] || name)
    .join(' ');
