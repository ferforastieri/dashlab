...
export function WidgetCard({
  ...
  const values: any = {
    ...
    PROMQL: [
      ...
      prometheusValue != null
        ? `${Number(prometheusValue).toFixed(Number(widget.config?.decimals ?? 1))}${widget.config?.suffix || ''}`
        : '—',
    ],
  };
  ...
  const metricLanes =
    widget.type === 'SYSTEM'
      ? [
          ...
          {
            label: 'Memória',
            value: metrics.memory == null ? '—' : `${metrics.memory.toFixed(0)}%`,
      ...
      : widget.type === 'NETWORK'
        ? [
            {
              label: 'Download',
              value: formatRate(metrics.download),
              points: history.download || [],
            },
  ...
  return (
    <div className={dashboardCn('widget')}>
      ...
      <div className={dashboardCn('widget-title')}>
        <Icon />
        {widget.title}
      </div>
      {metricLanes.length > 0 ? (
        ...
        <div className={dashboardCn('widget-values')}>
          {data.slice(1).map((value: any, index: number) => (
            <span
              key={index}
              className={typeof value === 'number' ? dashboardClassNames.metric : ''}
            >
...
function MetricLanes({ lanes }: { lanes: MetricLane[] }) {
  return (
    <div className="metric-lanes">
      {lanes.map((lane) => (
        <MetricChart key={`${lane.label}:${lane.detail || ''}`} lane={lane} />
      ))}
    </div>
  );
}
  ...
  return (
    <div className="metric-lane">
      <div className="metric-lane-head">
        <span title={lane.detail}>
          {lane.label}
          {lane.detail && <small>{lane.detail}</small>}
        </span>
        <strong>{lane.value}</strong>
      </div>
      <svg
        className={dashboardCn('metric-chart')}
        viewBox="0 0 100 36"
