...
describe('AuthService', () => {
  ...
  it('does not disclose whether login failed by user or password', async () => {
    ...
    await expect(service.login({ username: 'unknown', password: 'password1' })).rejects.toThrow(
  ...
  it('invalidates all refresh sessions after password change', async () => {
    ...
    db.user.findUniqueOrThrow.mockResolvedValue({ id: 'u1', passwordHash: hash });
    await expect(
      service.changePassword('u1', {
        currentPassword: 'old-password',
        newPassword: 'new-password',
      }),
    ).resolves.toMatchObject({ message: 'Senha alterada com sucesso' });
  ...
  it('prevents duplicate usernames case-insensitively', async () => {
    ...
    await expect(
      service.register({ username: 'Admin', password: 'password1' }),
