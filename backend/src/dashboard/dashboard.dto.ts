...
import { ArrayMaxSize, IsArray, IsBoolean, IsEnum, IsIn, IsInt, IsObject, IsOptional, IsString, IsUrl, Length, Max, MaxLength, Min, ValidateNested } from 'class-validator';
...
export class CreateApplicationDto {
  ...
  @IsUrl({ require_tld: false, protocols: ['http','https'], require_protocol: true }) @MaxLength(2048) url!: string;
  ...
  @IsOptional() @IsUrl({ require_tld: false, protocols: ['http','https'], require_protocol: true }) @MaxLength(2048) statusUrl?: string;
...
export class SaveLayoutDto {
  @IsArray() @ArrayMaxSize(200) @ValidateNested({ each: true }) @Type(() => LayoutItemDto) items!: LayoutItemDto[];
}
