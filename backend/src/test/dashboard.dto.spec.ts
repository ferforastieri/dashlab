...
import { CreateApplicationDto, CreateWidgetDto, SaveLayoutDto, SelectLayoutPresetDto } from '../dto/dashboard.dto';

describe('Dashboard DTO security boundaries', () => {
  it('rejects script protocols and accepts private homelab URLs', async () => {
    expect(await validate(plainToInstance(CreateApplicationDto, { name:'Bad', url:'javascript:alert(1)' }))).not.toHaveLength(0);
    expect(await validate(plainToInstance(CreateApplicationDto, { name:'Proxmox', url:'https://127.0.0.1:8006' }))).toHaveLength(0);
  });
  it('rejects unsupported widget types', async () => {
    expect(await validate(plainToInstance(CreateWidgetDto, { title:'X', type:'HTML', config:{} }))).not.toHaveLength(0);
  });
