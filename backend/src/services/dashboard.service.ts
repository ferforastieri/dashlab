...
import { BrandingDto, CreateApplicationDto, CreateWidgetDto, LayoutItemDto, UpdateApplicationDto, UpdateWidgetDto } from '../dto/dashboard.dto';
...
export class DashboardService {
  ...
  async get(userId: string, surface: "WEB" | "MOBILE") {
    ...
    if (effectivePreset !== 'FREE') d.layouts = await this.db.layoutItem.findMany({ where: { dashboardId: d.id, surface, preset: effectivePreset }, orderBy: { order: 'asc' } });
    if (!d.layouts.length) {
      await this.seedPreset(d.id, effectivePreset as any, surface);
      d.layouts = await this.db.layoutItem.findMany({ where: { dashboardId: d.id, surface, preset: effectivePreset }, orderBy: { order: 'asc' } });
    }
  ...
  async saveLayout(userId: string, surface: "WEB" | "MOBILE", items: LayoutItemDto[]) {
    ...
    await this.db.$transaction(async (tx) => {
      await tx.layoutItem.deleteMany({ where: { dashboardId: d.id, surface, preset: effectivePreset } });
  ...
  async metricsHistory(range:'15m'|'1h'|'6h'|'24h'='1h') {
    const base=process.env.PROMETHEUS_URL||'http://127.0.0.1:9090',seconds={"15m":900,"1h":3600,"6h":21600,"24h":86400}[range],end=Math.floor(Date.now()/1000),start=end-seconds,step=Math.max(15,Math.floor(seconds/60));
    ...
    await Promise.all(Object.entries(queries).map(async([key,q])=>{try{const json:any=await this.fetchJson(`${base}/api/v1/query_range?query=${encodeURIComponent(q as string)}&start=${start}&end=${end}&step=${step}`,5000);result[key]=(json?.data?.result?.[0]?.values||[]).map(([timestamp,value]:[number,string])=>({timestamp,value:Number(value)}));}catch{result[key]=[];}}));
    return result;
  }
